...
// IP address of Bear 1 (change this to Bear 1's IP)
IPAddress bear1IP(192, 168, 1, 1); 
...
void loop() {
  ...
  if (packetSize) {
    ...
    // If the message is "HUG", light up Bear 2's LED
    if (msg == "HUG") {
  ...
  if (temp > 30) {  // Threshold for detecting a hug
    // Send "HUG" message to Bear 1
    udp.beginPacket(bear1IP, UDP_PORT);
